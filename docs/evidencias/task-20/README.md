...
- Em 20/09, a seleção `municipio_cobertura = Aceguá` também chegou à Tela 2. O gráfico mensal ficou sem barras, confirmando a ausência de movimentações no recorte, mas exibiu um eixo de −1 a 1 e o mês como `-`, sem explicação do estado vazio. Falta configurar e retestar uma mensagem no próprio gráfico; as capturas novas continuam apenas na conversa.
...
- Aplicar e retestar os pontos das Telas 3 a 6 descritos abaixo, sem alterar os cálculos validados. Na Tela 2, falta comunicar o estado sem movimentações, testar a seleção de julho nas demais telas e arquivar a captura da versão exportada.
