...
- Na Tela 1, o filtro de meses foi ordenado cronologicamente e as dimensões dos mapas passaram a aparecer como “Município”. Uma captura com Porto Alegre selecionado mostra R$ 4,16 por pessoa, mas também revelou o zoom excessivo do mapa de pontos e a legenda de cor sem intervalo útil nessa seleção.
...
- Finalizar a Tela 1: legibilidade das duas legendas, zoom ao filtrar um município, escala de cores, nota de fonte/período e tooltip exato de Porto Alegre; guardar capturas após correção.
...
| 1 — Distribuição geográfica | A primeira captura mostrava `Mês` em ordem alfabética, `localizacao_mapa` e números em formato inglês. **Reteste parcial:** nova captura mostra meses em ordem cronológica e `Município` nos mapas. Com Porto Alegre selecionado, o mapa de pontos amplia até escala de 10 m e perde o contexto; o mapa de áreas mostra R$ 4,16/pessoa, mas sua legenda recalculada indica um intervalo vazio (`R$ 4,16 – < R$ 4,16`). O valor máximo da legenda de pontos continua truncado; o seletor `Mês` encobre parte do mapa. | No mapa de pontos, testar `Configurações do mapa → Comportamento do zoom → Nenhum` com vista inicial do RS e usar “Saldo líquido (R$ milhões)” com valores como `5,78`; se necessário, `Sum(valor)/1000000` só em “Dimensionar por”, preservando valor exato no tooltip. No mapa de áreas, testar escala de cor fixa de 0 a 1018.22 (máximo per capita na base completa), rótulo “R$ por pessoa” e números com duas casas sem repetir `R$` em cada faixa. Incluir nota de fonte, período e população 2024. Conferir tooltip de Porto Alegre, dois mapas completos e filtros de município, mês e recurso. |
