...
- Uma captura de 20/09 mostra `mes_nome = Julho` selecionado na Tela 1. Os mapas de pontos e áreas exibem municípios com movimentação nesse mês, confirmando visualmente a propagação do filtro. A legenda do mapa de áreas, porém, mostra apenas a faixa R$ 0,00–R$ 1.018,22; falta verificar se a cor diferencia os valores municipais nesse recorte. Na base processada, julho tem 105 municípios com movimentação e os valores por pessoa não são iguais (por exemplo, Porto Alegre: R$ 4,16; Pouso Novo: R$ 442,69). A escala global deve continuar fixa para permitir comparação entre recortes.
...
- Finalizar a Tela 1: incluir nota de fonte/período, guardar capturas da versão final e conferir se as cores distinguem valores por pessoa sem seleção e com julho selecionado. A equipe decidiu manter a formatação atual da legenda de pontos e do valor por pessoa no tooltip; isso não bloqueia a conclusão da tela. Manter os limites globais da legenda constantes para permitir comparação.
...
| 1 — Distribuição geográfica | A primeira captura mostrava `Mês` em ordem alfabética, `localizacao_mapa` e números em formato inglês. **Retestes parciais:** meses e rótulo “Município” corrigidos; com Porto Alegre selecionado, o ponto aparece em escala regional e o tooltip confirma código IBGE 4314902, R$ 5.782.558,14 e população 1.389.322. A escala de cor permanece em 0–R$ 1.018,22 com a cidade selecionada. Nova captura sem seleções mostra as legendas na horizontal com todos os limites visíveis. Os tooltips exibem valores distintos por município. A legenda de pontos ainda exibe `R$ 5,782,558.14` e o tooltip exibe `R$4.16`; a equipe optou por manter essa formatação. **Estado vazio corrigido:** com Aceguá selecionado na Tela 3, os dois mapas permanecem na região do RS e exibem “Sem movimentações FUNDEC nesta base para a seleção atual.” **Reteste com dados:** Porto Alegre mostra o ponto e a área municipal preenchida. Com `mes_nome = Julho`, os dois mapas exibem municípios com movimentação no mês, mas a legenda de áreas mostra uma única faixa R$ 0,00–R$ 1.018,22. | Incluir nota de fonte, período e população 2024; conferir valores no tooltip e a diferenciação de cores com julho selecionado; retestar recurso e guardar as capturas finais do QVF exportado. |
...
Em teste relatado após a última captura da Tela 1, a legenda do mapa de áreas permaneceu igual ao selecionar municípios diferentes, enquanto o tooltip mostrou valores diferentes. Os limites globais fixos de 0 a R$ 1.018,22 são esperados e permitem comparar municípios na mesma escala. A seleção de julho inclui muitos municípios, mas a captura mostra uma só faixa nessa legenda; isso ainda não comprova que as cores estejam diferenciando os valores. Falta guardar uma captura comparativa de municípios de extremos diferentes, por exemplo Porto Alegre (R$ 4,16/pessoa) e Coqueiro Baixo (R$ 1.018,22/pessoa), e verificar se a cor em modo sem seleção e no recorte de julho comunica a diferença.
