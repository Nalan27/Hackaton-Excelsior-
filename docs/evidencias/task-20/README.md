...
| 2 — Linha do Tempo | Nova captura sem filtros confirma que o eixo ainda mostra `ano_mes_ordem` e `202405`–`202409`; o eixo de valores não explicita R$. As três séries continuam visíveis e em ordem cronológica. | Exibir `Maio/2024`–`Setembro/2024` em ordem cronológica e informar que a escala é em reais. Como `mes_nome` já contém nomes em português, uma dimensão calculada simples é `=Dual(mes_nome & '/' & Floor(ano_mes_ordem/100), ano_mes_ordem)`, com rótulo “Mês do repasse” e ordenação numérica ascendente. Confirmar novamente as cinco linhas mensais e o filtro de julho depois da troca. |
