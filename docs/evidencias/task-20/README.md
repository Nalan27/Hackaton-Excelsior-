...
| 1 — Distribuição geográfica | O filtro `Mês` aparece em ordem alfabética (Agosto, Julho, Junho, Maio, Setembro); as legendas mostram `localizacao_mapa`, `Valor Recebido` e números como `5.78M` ou `R$873.17`; a tela não informa explicitamente a fonte do FUNDEC, o período nem o ano da população. | Ordenar `mes_nome` por `Min(ano_mes_ordem)` crescente; rotular a localização como “Município”, o tamanho dos pontos como “Saldo líquido (R$)” e a cor das áreas como “Valor líquido por habitante (R$)”. Formatar os limites das legendas com vírgula decimal e ponto de milhar; manter as expressões das medidas. Incluir nota de fonte, período e população 2024. Conferir tooltip de Porto Alegre e ambos os mapas após filtros de município, mês e recurso. |
