...
Revisão iniciada em 19/09/2026 sobre o [snapshot 11](../../../qlik/versoes-do-app/11-task-19/README.md) e as [capturas das Telas 0 a 6](../../screenshots/tela-6-resumo/). As capturas são do aplicativo antes da exportação de 18/09. Ainda falta testar a cópia que será usada nos prints e no pitch, aplicar as correções abaixo e exportar um **novo QVF com dados**. Este documento não atesta a conclusão da issue.
...
| 3 — Cobertura | A tabela mostra `Clique para incluir um título` e códigos como `nao_listado_sem_registro`. | Dar título à tabela e apresentar classificações legíveis, mantendo os códigos originais na base. Conferir que 478 / 334 / 144 continuam iguais. |
...
| 5 — Vulnerabilidade | A tabela mostra `Clique para incluir um título`. | Nomear a tabela; conferir que o gráfico mantém 333 municípios comparáveis e a nota de fonte/anos. |
...
| Navegação | As capturas mostram setas entre pastas, mas não comprovam a sequência final nem o efeito das seleções ao mudar de tela. | Deixar as sete telas de apresentação na ordem 0–6; retirar páginas de teste/validação do percurso público; percorrer as sete telas com seleções limpas e depois com uma seleção ativa. |

Na [Tela 0](../../screenshots/tela-6-resumo/tela-0-visao-geral.png), o KPI “Total Repassado” representa o **saldo líquido**, após os 18 ajustes negativos. Renomear para “Total líquido repassado” elimina essa ambiguidade. Conferir também se as Telas 1, 2, 4 e 6 mostram ou remetem claramente às fontes, ao período e às unidades sem depender da explicação oral do pitch. A Tela 0 já traz fonte e período; a Tela 5 identifica fontes e anos.
